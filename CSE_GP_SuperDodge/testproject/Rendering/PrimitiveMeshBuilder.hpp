...
public:
    static Mesh CreateRect(const Vector2& center, const Vector2& size, const Color& color)
    {
    ...
    static Mesh CreateCircle(const Vector2& center, float radius, const Color& color)
    {
        ...
        for (int i = 0; i < SegmentCount; ++i)
        {
            ...
            AddTriangle(
                vertices,
                center,
                Vector2(center.x + std::cos(angle1) * radius, center.y + std::sin(angle1) * radius),
                Vector2(center.x + std::cos(angle2) * radius, center.y + std::sin(angle2) * radius),
                vertexColor);
        }
        ...
        const Color& color)
    {
        ...
        for (int i = 0; i < EdgeCount; ++i)
        {
            ...
            AddTriangle(
                vertices,
                center,
                Vector2(center.x + std::cos(angle1) * radius1, center.y + std::sin(angle1) * radius1),
                Vector2(center.x + std::cos(angle2) * radius2, center.y + std::sin(angle2) * radius2),
                vertexColor);
        }
    ...
    static Mesh CreateHeart(const Vector2& center, float size, const Color& color)
    {
        constexpr int SegmentCount = 32;
        const DirectX::XMFLOAT4 vertexColor(color.r, color.g, color.b, color.a);
        std::vector<Vertex> vertices;
        ...
        for (int i = 0; i < SegmentCount; ++i)
        {
            AddTriangle(
                vertices,
                center,
                GetHeartPoint(center, size, 2.0f * Pi * i / SegmentCount),
                GetHeartPoint(center, size, 2.0f * Pi * (i + 1) / SegmentCount),
                vertexColor);
        }
...
private:
    ...
    static Vector2 GetHeartPoint(const Vector2& center, float size, float angle)
    {
        ...
        const float y = 13.0f * std::cos(angle)
            - 5.0f * std::cos(2.0f * angle)
            - 2.0f * std::cos(3.0f * angle)
            - std::cos(4.0f * angle);

        return Vector2(
            center.x + x * size / 32.0f,
            center.y - y * size / 32.0f);
    }
